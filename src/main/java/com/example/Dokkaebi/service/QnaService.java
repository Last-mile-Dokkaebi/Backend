...
public class QnaService {
    ...
    public Qna register(QnaReqDto qnaReqDto){
        Member member = memberRepository.findOne(qnaReqDto.getMemberId());
        qnaReqDto.setRegiDate(LocalDateTime.now());
        Qna qna = qnaReqDto.toEntity(member,null);
        qnaRepo.save(qna);
        return qna;
    }
    ...
    public void reply(QnaReqDto qnaReqDto) throws Exception {
        Member admin = memberRepository.findOne(qnaReqDto.getAdminId());
        Qna qna = qnaRepo.findByQnaId(qnaReqDto.getQnaId());
        qna.RegiRepliedAdmin(admin,qnaReqDto.getComment());
        qnaRepo.save(qna);
    }
